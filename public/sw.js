...
self.addEventListener('fetch', (event) => {
  ...
  if (method === 'GET' && requestedPath.startsWith('/webgal/game/bgm/')) {
    const musicFileName = requestedPath.substring('/webgal/game/bgm/'.length);
       ...
       // console.log(`[SW BGM] Intercepting ${method} ${requestedPath}. Handling background music.`);
       event.respondWith(
        (async () => {
            let dynamicTitle;
             try {
          ...
          const targetPathForReadFile = `/data/${dynamicTitle}/music/${musicFileName}`;
           // console.log(`[SW BGM] Attempting to load music from IndexedDB: ${targetPathForReadFile}`);

          try {
            // Attempt to read from IndexedDB
            const fileContent = await readFile(targetPathForReadFile);

            // --- START: Corrected Range Request Handling for BGM ---
            if (!(fileContent instanceof Blob)) {
                console.error(`[SW BGM] Expected Blob but got ${typeof fileContent} for ${targetPathForReadFile}`);
                return createErrorResponse(500, 'Internal Server Error', 'Invalid data type retrieved from IndexedDB for BGM.');
            }

            const blob = fileContent;
            const totalSize = blob.size;
            // Get content type: prioritize blob.type, then guess from extension, default if needed
            const contentType = blob.type || getMimeTypeFromExtension(targetPathForReadFile) || 'audio/mpeg'; // Default to mpeg for bgm? Or octet-stream

            const rangeHeader = event.request.headers.get('Range');

            if (rangeHeader) {
                // Parse the Range header (using the helper function defined elsewhere)
                const range = parseRangeHeader(rangeHeader, totalSize);
                ...
                if (range && range.start < totalSize) {
                    ...
                    // console.log(`[SW BGM] Serving 206 Partial Content: Range ${range.start}-${range.end}/${totalSize}, Size: ${chunkSize}`);
                    return new Response(slicedBlob, { status: 206, headers });

                } else {
                    // Invalid Range header format or range start beyond file size
                    console.warn(`[SW BGM] Invalid Range header "${rangeHeader}" for size ${totalSize}. Falling back to 200 OK.`);
                    // Respond with 200 OK instead of 416 for simplicity
                    const headers = {
                        'Content-Type': contentType,
                        'Content-Length': totalSize,
                        'Accept-Ranges': 'bytes' // Still accept ranges even if this specific one was bad
                ...
                // No Range header, return the entire file (200 OK)
                // console.log(`[SW BGM] Serving 200 OK: Size ${totalSize}`);
                const headers = {
                    'Content-Type': contentType,
                    'Content-Length': totalSize,
                    'Accept-Ranges': 'bytes' // Indicate we support ranges for future requests
            ...
            // --- END: Corrected Range Request Handling for BGM ---

          } catch (idbError) {
            if (idbError.name === 'FileNotFoundError') {
               console.warn(`[SW BGM] Music ${requestedPath} (path: ${targetPathForReadFile}) not found in IndexedDB. Falling back to network.`);
               try {
                 // Fallback: Fetch from the network
                 const networkResponse = await fetch(event.request);
                   // console.log(`[SW BGM] Network fallback for ${requestedPath} status: ${networkResponse.status}`);
                 return networkResponse; // Return network response (success or error)
               } catch (networkError) {
                  console.error(`[SW BGM] Network fallback failed for ${requestedPath}:`, networkError);
                    return createErrorResponse(503, 'Network Error',`Failed to fetch resource: ${networkError.message}`);
               }
            } else { // Handle other IndexedDB errors
                 if (idbError.message && idbError.message.includes('Database open is blocked')) {
                     return createErrorResponse(503, 'Database Access Blocked','Please close other tabs using this app.');
                      ...
                      // Include specific path in error message
                      console.error(`[SW BGM] Failed to process BGM request for ${targetPathForReadFile}:`, idbError);
