...
Object.defineProperty(String.prototype, 'isDigit', {
   ...
   * @returns {Promise<[string, string]>} - [cap_key, id]
   */
  async function requestCapKey(access_token, user_id, service_id) {
    try {
      // Create application
  ...
  export async function get_all_keys(cap_api_key) {
    try {
      ...
      // Request cap key
      const [capkey, capid] = await requestCapKey(access_token, user_id, "018c653e-4f1b-433f-82f9-732ef2767040");
      
      // Request suno key
      const [sunokey, sunoid] = await requestCapKey(access_token, user_id, "f2b646d8-3cfd-46ef-969a-1ea9eebde329");
  ...
  export async function gettoken(kind) {
    try {
      ...
      const [status, capkey, capid, sunokey, sunoid, platformkey] = await get_all_keys(verify_token);
