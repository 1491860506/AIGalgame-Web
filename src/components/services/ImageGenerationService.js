...
export async function runMainProcess(prompt1, prompt2, kind, cover = 0, updateStatus = console.log) {
    ...
    if (!config) {
        updateStatus("AI_draw Error: Failed to load config.");
    ...
    try {
        ...
        updateStatus(`AI_draw Error creating directories: ${e.message}`);
    ...
    // --- Get Tasks from GPT ---
    updateStatus("AI_draw: Requesting task list from GPT...");
    let drawingTasks = null;
    const gptId = randint(1, 100000); // Assuming randint is available
    let gptSuccess = false;
    for (let i = 0; i < 3; i++) {
        if (cancellationSignal.cancelled) { updateStatus("AI_draw Error: Operation cancelled before GPT."); await gptDestroy(gptId).catch(() => { }); return { error: "Operation cancelled" }; }
        try {
            const gpt_response = await gpt(prompt1, prompt2, kind, gptId);
            if (cancellationSignal.cancelled) { updateStatus("AI_draw Error: Operation cancelled during GPT."); await gptDestroy(gptId).catch(() => { }); return { error: "Operation cancelled" }; }

            if (gpt_response === 'over_times') { updateStatus(`AI_draw: GPT attempt ${i + 1} failed (internal retries).`); await sleep(1000); continue; }
            if (gpt_response && gpt_response !== 'error') {
                ...
                if (drawingTasks !== null) { await gptDestroy(gptId); gptSuccess = true; updateStatus(`AI_draw: GPT returned ${drawingTasks.length} initial tasks.`); console.log("Initial GPT Tasks:", drawingTasks); break; }
                else { updateStatus(`AI_draw: GPT attempt ${i + 1} parse failed.`); drawingTasks = null; await sleep(1000); }
            } else { updateStatus(`AI_draw: GPT attempt ${i + 1} failed.`); await sleep(1000); }
        } catch (e) { updateStatus(`AI_draw: GPT attempt ${i + 1} error: ${e.message}`); console.error(`GPT call failed on attempt ${i + 1}:`, e); await sleep(1000); }
    }

    // --- Early Exit Checks (AFTER GPT attempts) ---
    if (!gptSuccess) { await gptDestroy(gptId).catch(() => { }); updateStatus("AI_draw Error: Failed GPT task list."); return { error: "GPT task list failed" }; }
    if (!drawingTasks) { updateStatus("AI_draw Error: GPT response parsing failed. No tasks to process."); return { success: [], failed: [], skipped: [] }; }
    if (cancellationSignal.cancelled) { updateStatus("AI_draw Error: Operation cancelled after GPT."); return { error: "Operation cancelled" }; }
    ...
    if (totalTasks === 0) { updateStatus("AI_draw Error: GPT returned no unique tasks."); return { success: [], failed: [], skipped: [] }; }
    updateStatus(`AI_draw: Processing ${totalTasks} unique tasks.`);

    // --- Initialize Main Model Manager ---
    updateStatus("AI_draw: Initializing main model manager...");
    ...
    if (mainModelManager.terminate_flag) {
        updateStatus("AI_draw Error: Main model manager could not initialize any valid models.");
    ...
    // --- Prepare Task Queue ---
    const taskQueue = [];
    updateStatus("AI_draw: Adding tasks to queue...");
    ...
    updateStatus(`AI_draw: Starting ${numWorkers} main workers...`);
    ...
    updateStatus("AI_draw: Monitoring progress...");
    ...
    while (true) {
        ...
        if (!cancellationSignal.cancelled && finalAccountedCount >= totalTasks && pendingSplitCount === 0 && activeSplitCount === 0) {
            updateStatus(`AI_draw: ${realkind} All ${totalTasks} tasks completed normally. Finishing up.`);
        ...
        if (!cancellationSignal.cancelled && mainModelManager.terminate_flag) {
            updateStatus(`AI_draw: ${realkind} Main model manager exhausted. Signaling cancellation...`);
        ...
        if (cancellationSignal.cancelled) {
            ...
            if (!normalCompletionReached) {
                updateStatus("AI_draw: Cancellation signalled. Stopping monitoring loop.");
        ...
        if (Date.now() - lastStatusUpdateTime > 4000) {
            updateStatus(`AI_draw: ${realkind} Progress: ${finalAccountedCount}/${totalTasks}. MainQ: ${mainQueueLength}. PendingSp: ${pendingSplitCount}. ActiveSp: ${activeSplitCount}. MainMgrTerm: ${mainModelManager.terminate_flag}. Cancelled: ${cancellationSignal.cancelled}.`);
    ...
    updateStatus(`AI_draw: Waiting for final operations... Cancelled: ${cancellationSignal.cancelled}`);
    ...
    updateStatus("AI_draw: Compiling final results...");
    ...
    updateStatus(`AI_draw: Processing Complete. Cancelled: ${cancellationSignal.cancelled}`);
...
export async function getAllPersonsImagesJS(updateStatus = console.log) {
    updateStatus("AI_draw: Starting: Get All Persons Images");
...
export async function getSinglePersonImageJS(characterNames, updateStatus = console.log) {
    updateStatus(`AI_draw: Starting: Get Single Person Image for ${characterNames}`); // Update log to show it's a list or single name
    ...
    try {
        ...
        if (!characterNames || characterNames.length === 0) {
            updateStatus("AI_draw: Character name list is empty, returning.");
        ...
        if (characterNames.length === 1) {
            ...
            if (!characterInfo) {
                updateStatus(`AI_draw Error: Character '${characterName}' not found in ${characterJsonPath}`);
                return { error: `Character ${characterName} not found` };
            }

            // Prepare prompts for GPT
            updateStatus("AI_draw: Processing prompt for single character...");
            ...
            // Call main process
            updateStatus(`AI_draw: Generating image for ${characterName}...`);
            return await runMainProcess(prompt1, prompt2, 'character', true, updateStatus); // cover = 1 (true)
        }
        ...
        // Prepare prompts for GPT
        updateStatus("AI_draw: Processing prompt for multiple characters...");
        ...
        // Call main process
        updateStatus(`AI_draw: Generating image for multiple characters: ${characterNames.join(', ')}...`);
        return await runMainProcess(prompt1, prompt2, 'character', true, updateStatus); // cover = 1 (true)

    } catch (error) {
        updateStatus(`AI_draw Error processing character(s) ${characterNames}: ${error.message}`); // Update error message
...
export async function getPlacesImagesJS(cover = 0, updateStatus = console.log) {
    updateStatus("AI_draw: Starting: Get Places Images");
    const config = loadConfigJS();
    if (!config) return { error: "Config load failed" };
    ...
    try {
        updateStatus("AI_draw: Reading places data...");
        ...
        try {
            ...
            if (JSON.stringify(placeData) === '[]') {
                updateStatus("AI_draw: No places defined in place.json. Skipping background generation.");
...
export async function getTitleImagesJS(cover = 0, updateStatus = console.log) {
    ...
    try {
        updateStatus("AI_draw: Processing prompt for title image...");
        const [prompt1, prompt2] = await processPrompt('首页背景生成');

        updateStatus("AI_draw: Generating title images...");
