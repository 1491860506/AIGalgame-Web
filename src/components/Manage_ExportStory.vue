        ...
        <div class="tree-container">
          ...
          <div v-else class="tree-content">
            ...
            <ul class="tree" v-if="rootNode">
              <story-node :node="rootNode" :depth="0" @view="viewStory" @export="prepareExport"
      ...
      <section class="preview-section card" :class="{ active: selectedStory }">
        <div class="preview-header">
          ...
          <div class="format-controls">
            <button v-for="format in previewFormats" :key="format.id"
              :class="['btn btn-sm btn-outline format-btn', { 'active-format': currentPreviewFormat === format.id }]"
              @click="currentPreviewFormat = format.id">
              {{ format.name }}
            </button>
          </div>
          <button class="btn btn-text btn-sm close-btn" @click="selectedStory = null" aria-label="Close preview"
            title="关闭预览">
        ...
        <div v-if="selectedStory" class="preview-footer">
          <div class="export-options">
            ...
            <div class="export-buttons">
              <button v-for="format in exportFormats" :key="format.id" class="btn btn-primary export-btn"
                :class="format.class" @click="exportStory(format.id)" :disabled="isExporting">
...
export default {
  ...
  generateHtmlDocument() {
    ...
    // storyHtmlContent already contains its own <style> block and <div class="story-preview-container">...</div>
    // The goal is to wrap this in a nicer page structure with dark mode support
    // that also correctly provides CSS variables for storyHtmlContent's internal styles.
...
<head>
  ...
  <style>
    :root {
      ...
      /* Colors */
      --theme-bg: #f8f9fa;                     /* Page background */
      --theme-text: #212529;                  /* Default text */
      --theme-primary: #007bff;               /* Primary action/highlight color */
      --theme-primary-text: #ffffff;          /* Text on primary background */
      --theme-secondary-text: #6c757d;        /* Muted text */
      --theme-border: #dee2e6;                /* Borders */
      --theme-surface: #ffffff;               /* Card/element backgrounds */
      --theme-hover-bg: #e9ecef;              /* Subtle hover background */
      --theme-shadow-color: rgba(0, 0, 0, 0.075);

      /* Typography */
      --font-sans: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", "Liberation Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji", "Noto Sans SC", "PingFang SC", "Microsoft YaHei";
      ...
      /* --- Variables for storyHtmlContent compatibility --- */
      /* These names must match those used INSIDE formattedStoryHtml's style block */
      --text-primary: var(--theme-text);
      --content-bg: var(--theme-surface);        /* Story container's own background */
      --primary-color: var(--theme-primary);
      --border-color: var(--theme-border);
      --surface-color: var(--theme-surface);     /* For elements like dialog bubbles within story */
      --hover-overlay: var(--theme-hover-bg);    /* Used for narration bg in formattedStoryHtml */
      --text-secondary: var(--theme-secondary-text);
      --border-radius-sm: var(--radius-sm);
      --border-radius-md: var(--radius-md);
      /* --border-radius-lg: var(--radius-lg); /* If formattedStoryHtml uses it */
    }

    @media (prefers-color-scheme: dark) {
      :root {
        /* --- Global Theme Variables (Dark Mode) --- */
        --theme-bg: #0d1117;                     /* Very dark, GitHub-like */
        --theme-text: #c9d1d9;                  /* Light gray text */
        --theme-primary: #58a6ff;               /* Brighter blue for dark mode */
        --theme-primary-text: #0d1117;          /* Dark text on primary bg */
        --theme-secondary-text: #8b949e;        /* Muted gray */
        --theme-border: #30363d;                /* Darker border */
        --theme-surface: #161b22;               /* Dark surface for cards */
        --theme-hover-bg: #21262d;              /* Darker hover */
        --theme-shadow-color: rgba(0, 0, 0, 0.25);

        /* --- Variables for storyHtmlContent compatibility (Dark) --- */
        --text-primary: var(--theme-text);
        --content-bg: var(--theme-surface);
        --primary-color: var(--theme-primary);
        --border-color: var(--theme-border);
        --surface-color: var(--theme-surface);
        --hover-overlay: var(--theme-hover-bg);
        --text-secondary: var(--theme-secondary-text);
      }
    }
    ...
    *::after {
    ...
    .export-header .meta-info {
      font-size: calc(var(--spacing-unit) * 0.875);
      color: var(--theme-secondary-text);
    }

    .export-header .meta-info span {
      margin-right: var(--spacing-unit);
    }
    .export-header .meta-info span:last-child {
      margin-right: 0;
    }

    /* --- Main Content Styles --- */
    /* The storyHtmlContent contains .story-preview-container.
       Its styles are defined within storyHtmlContent itself.
       We mainly ensure it fits well.
    */
    .export-main-content {
      /* If .story-preview-container inside storyHtmlContent doesn't have its own shadow/border,
         you might want to add some styling to .export-main-content or make
         .story-preview-container use a global var like --theme-shadow-color */
      background-color: var(--content-bg); /* This should be picked up by .story-preview-container if it uses var(--content-bg) */
      border-radius: var(--radius-lg);
      box-shadow: 0 4px 12px var(--theme-shadow-color);
      overflow: hidden; /* If storyHtmlContent has elements that might overflow the rounded corners */
    }
    /* If storyHtmlContent's root .story-preview-container does NOT have padding, add it here */
    /* .story-preview-container { padding: var(--spacing-unit); } /* Example */
    ...
    /* --- Styling for content coming from formattedStoryHtml --- */
    /* The actual styles for .story-place, .dialog-bubble etc. are expected to be *inside* storyHtmlContent.
       The CSS variables defined in :root should allow those internal styles to adapt to the theme.
       If formattedStoryHtml's styles are very specific and don't use these variables,
       they might not fully adapt. This solution relies on variable-based styling from formattedStoryHtml.
    */

    /* Ensure images within story content are responsive */
...
<body>
  <div class="export-page-wrapper">
    ...
    <main class="export-main-content">
      ${storyHtmlContent}
    </main>
...
.content-wrapper {
  display: flex;
  gap: 20px;
  /* Space between tree and preview sections */
  flex-grow: 1;
...
.choice-tree-section {
  ...
  min-width: 280px;
  max-height: 500px;
  /* Or adjust as needed, e.g., calc(100vh - someOffset) for modals */
}
...
.tree-container {
  ...
  overflow-y: auto;
  overflow-x: auto;
  /* MODIFIED: Added horizontal scrolling */
  /* padding-right: 10px; /* Consider if padding is needed with scrollbars */
}
...
.tree {
  ...
  padding: 0;
  margin: 0;
  /* MODIFIED: Allow the tree to grow as wide as its content for horizontal scrolling */
  min-width: 100%;
  width: max-content;
  padding-bottom: 10px;
  /* Optional: space at the bottom if items are close to scrollbar */
}
...
.preview-body {
  ...
  overflow-y: auto;
  /* Enable scrolling for preview content */
  padding-bottom: 15px;
  padding-right: 10px;
  /* Space for scrollbar */
  margin-right: -10px;
...
.placeholder-icon {
  /* Specific to preview placeholder */
  font-size: 2.5rem;
  margin-bottom: 15px;
  color: var(--text-tertiary);
}
...
.loading-overlay {
  position: absolute;
  /* Position relative to parent modal content */
  inset: 0;
  /* Cover the whole modal content */
  background-color: rgba(var(--surface-color-rgb, 249, 249, 249), 0.8);
  /* Use surface color with opacity */
  display: flex;
  justify-content: center;
  align-items: center;
  z-index: 1;
  /* Above preview content but below modal header/footer */
...
@media (max-width: 768px) {
  ...
  .preview-section {
    flex: none;
    /* Remove flex constraints when stacked */
    min-width: 0;
  }

  .preview-section {
    order: -1;
    /* Place preview above tree on mobile */
    opacity: 1;
    /* Always active when stacked */
    pointer-events: auto;
  }
