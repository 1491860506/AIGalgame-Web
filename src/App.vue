    ...
    <div class="main-content">
      <div class="content">
        <transition name="fade" mode="out-in">
          <router-view @show-message="showMessageBubble" />
        </transition>
      </div>
    </div>
  </div>
...
(function() {
  ...
  initializeDB().catch(error => {
    ...
    originalConsoleError.call(console, "Failed to initialize IndexedDB, falling back to localStorage:", error);
  });
  ...
  function storeLog(type, message) {
    ...
    if (db) {
      ...
      // IndexedDB not ready yet, use localStorage as fallback
      fallbackToLocalStorage(logEntry);
    }
  }
  ...
  function fallbackToLocalStorage(logEntry) {
    try {
      const localStorageKey = 'capturedConsoleLogs';
      let logs = [];
      try {
      ...
      // Limit localStorage logs to prevent exceeding quota
      if (logs.length > 100) {
        logs = logs.slice(-100);
      }
      ...
      originalConsoleError.call(console, "Failed to store log to localStorage fallback:", e);
    }
  }
  ...
  window.getStoredLogs = function() {
    return new Promise((resolve, reject) => {
      if (!db) {
        // If IndexedDB isn't ready, try localStorage
        try {
          const localStorageKey = 'capturedConsoleLogs';
          const storedLogs = localStorage.getItem(localStorageKey);
          resolve(storedLogs ? JSON.parse(storedLogs) : []);
        } catch (e) {
          reject(e);
        }
        return;
  ...
  window.clearStoredLogs = function() {
    return new Promise((resolve, reject) => {
      if (!db) {
        // If IndexedDB isn't ready, try clearing localStorage
        try {
          localStorage.removeItem('capturedConsoleLogs');
          resolve();
        } catch (e) {
          reject(e);
        }
  ...
  // Migration from localStorage to IndexedDB (optional)
  function migrateFromLocalStorage() {
    ...
    try {
      ...
      originalConsoleError.call(console, "Failed to migrate logs from localStorage:", e);
    }
  }
  
  // Attempt migration after DB is initialized
  initializeDB().then(() => {
